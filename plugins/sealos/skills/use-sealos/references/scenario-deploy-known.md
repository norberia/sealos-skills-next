...
3. **Inspect the hit** — which args are required:
   ```bash
   python3 scripts/sealos-api.py store-get <template>
   ```
...
5. **Deploy**:
   ```bash
   python3 scripts/sealos-api.py deploy-store <template> \
     [--args-json '{"KEY":"value"}']
   ```
6. **Verify**:
   ```bash
...
- No store hit → check [recipes.md](recipes.md) for a ready image recipe,
  else use the official Docker image and follow the template steps of the
  project scenario ([scenario-deploy-project.md](scenario-deploy-project.md),
  steps 4-7 — no building). **Never build a product from source when an
